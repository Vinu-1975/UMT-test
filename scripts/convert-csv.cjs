/* CSV → dashboard JSON conversion.
   ...
   - src/lib/data/vdi-users.json
   - src/lib/data/domain-records.json
...
fs.writeFileSync(path.join(OUT_DIR, "vdi-users.json"), JSON.stringify(vdiUsers));
...
fs.writeFileSync(path.join(OUT_DIR, "domain-records.json"), JSON.stringify(domainRecords));
